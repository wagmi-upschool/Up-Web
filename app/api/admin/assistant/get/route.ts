...
export async function GET(req: NextRequest) {
    ...
    try {
        ...
        const lambda = await fetch(`${process.env.REMOTE_URL}/assistant/get${assistantId ? `/${assistantId}` : ''}?sortByCreatedAt=desc`, {
            method: 'GET',
            headers: {
        ...
        return NextResponse.json({ error: 'Failed to get assistant', details: error?.message || 'Unknown error' }, { status: 500 });
    }
}