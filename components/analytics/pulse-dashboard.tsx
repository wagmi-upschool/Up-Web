...
type PulseDashboardSummary = Pick<
  DashboardSummaryResponse,
  "totalFeedbacks" | "overallAverageRating" | "maxRating" | "hourlyRatings" | "cultureScore"
>;

type PulseDashboardProps = {
  companyName: string;
  summary?: PulseDashboardSummary;
  isLoading: boolean;
...
function FeedbackPageShell({ children }: { children: ReactNode }) {
  return (
    ...
    >
      <div
        aria-hidden="true"
        className="pointer-events-none fixed inset-0 z-0 bg-[radial-gradient(circle_at_top_left,_#0057FF_0,_transparent_28%),radial-gradient(circle_at_top_right,_#985DF8_0,_transparent_24%),radial-gradient(circle_at_bottom_left,_#00D9C0_0,_transparent_20%),linear-gradient(180deg,_#F8F2E7_0%,_#F3EAD7_100%)] opacity-[0.08]"
      />
      <div
        aria-hidden="true"
              ...
              <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="sm:text-right">
                  ...
                  <p className="mt-2 font-poppins text-sm uppercase tracking-[0.22em] text-[#171717]/38">
                    / {formatScore(summary?.cultureScore.maxRating ?? 4)} toplam
                  </p>
                </div>
              </div>
              ...
              {!hasRequestedData ? (
                <div className="rounded-2xl border border-[#171717]/10 bg-[#FFFFFF]/60 p-6 text-center">
                  <p className="font-poppins text-base text-[#171717]/72">
                    İklim değerlendirme bölümü sorgu parametresi geldiğinde dolacak.
                  </p>
                </div>
              ) : isLoading ? (
                <LoadingState />
                ...
                <EmptyState
                  description="API geçerli yanıt döndü ancak kültür sorusu verisi bulunamadı."
                  icon={<Users className="h-10 w-10 text-[#FC7700]/55" />}
                  title="Soru verisi yok"
                />
              ) : (
                <div className="space-y-3">
          ...
          <div className="rounded-[26px] border border-[#171717]/8 bg-[linear-gradient(180deg,#FFFDF8_0%,#F8F2E7_100%)] p-4 transition-all duration-300 group-hover:border-white/35 group-hover:bg-[linear-gradient(180deg,rgba(255,255,255,0.62)_0%,rgba(248,242,231,0.55)_100%)] group-hover:backdrop-blur-lg">
            ...
            {isLoading ? (
              ...
              <EmptyState
                ...
                icon={<Activity className="h-10 w-10 text-[#0057FF]/55" />}
                title={missingDataTitle}
              />
            ) : trendData.length === 0 ? (
              <EmptyState
                description="API geçerli yanıt döndü ancak `hourlyRatings` boş geldi. Tasarım gereği boş trend durumu render edildi."
                icon={<BarChart3 className="h-10 w-10 text-[#985DF8]/60" />}
