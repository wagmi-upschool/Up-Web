...
const normalizeServiceAccount = (
  ...
  if (!clientEmail) {
    throw new Error(
      "Firebase service account is missing clientEmail/client_email"
    );
  }
  if (!privateKey) {
    throw new Error(
      "Firebase service account is missing privateKey/private_key"
    );
  }
...
const getServiceAccount = (): LoadedServiceAccount => {
  try {
    ...
    if (rawServiceAccount) {
      ...
      const normalized = rawServiceAccount.trim().startsWith("{")
        ? rawServiceAccount
        : Buffer.from(rawServiceAccount, "base64").toString("utf8");
      return normalizeServiceAccount(
        JSON.parse(normalized) as RawServiceAccount
      );
    }
    ...
    if (
      ...
      console.log(
        "🔑 Constructing Firebase service account from individual environment variables"
      );
      ...
      if (!privateKey.includes("-----BEGIN PRIVATE KEY-----")) {
        try {
          ...
          throw new Error(
            "Invalid private key: not valid base64 or PEM format"
          );
        }
      }
    ...
    throw new Error(
      "Firebase service account env vars are missing. Set FIREBASE_SERVICE_ACCOUNT_KEY or the individual FIREBASE_* credentials."
    );
  } catch (error) {
...
export async function getRemoteConfigValue(
  ...
  try {
    ...
    console.error(
      `❌ Error retrieving Remote Config parameter '${parameter}':`,
      error
    );
    return null;
  }
...
export async function setRemoteConfigValue(
  ...
  try {
    if (!remoteConfig) {
      console.warn(
        "⚠️ Firebase Remote Config is unavailable; cannot set parameter."
      );
      return false;
    }
    ...
    console.log(
      `✅ Remote Config parameter '${parameter}' updated. Version: ${updatedTemplate.version}`
    );
    return true;
  } catch (error) {
    console.error(
      `❌ Error setting Remote Config parameter '${parameter}':`,
      error
    );
    return false;
  }
