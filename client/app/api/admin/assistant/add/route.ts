...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const { name, id, title, src, template, description, introduction, prompt, temperature, topP, maxTokens, frequencyPenalty, presencePenalty, modelName, extra } = body;
        
        const type = introduction.filter((f: any) => f.type === "user-input").length > 0 ? "user-input" : 'default';
        ...
        return NextResponse.json({ error: 'Failed to add assistant', details: error?.message || 'Unknown error' }, { status: 500 });
    }
}